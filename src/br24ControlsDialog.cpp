...
EVT_BUTTON(ID_INTERFERENCE_REJECTION, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_TARGET_BOOST, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_BEARING_ALIGNMENT, br24ControlsDialog::OnControlButtonClick)
...
EVT_BUTTON(ID_GAIN, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_SEA, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_RAIN, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_FTC, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_STC_PRESET, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_TUNE, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_TUNE_COARSE, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_STC_CURVE, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_MBS_ENABLED, br24ControlsDialog::OnControlButtonClick)
EVT_BUTTON(ID_DISPLAY_TIMING, br24ControlsDialog::OnControlButtonClick)
...
EVT_COMMAND(ID_CONTROL_DIALOG_REFRESH, wxEVT_COMMAND_TEXT_UPDATED, br24ControlsDialog::OnRefreshValues)
...
wxString interference_rejection_names[3] = { _("Off"), _("Normal"), _("High") };;
wxString target_boost_names[3] = { _("Off"), _("Low"), _("High") };
wxString timed_idle_times[8] = { _("Off"), _("5 min"), _("10 min"), _("15 min"), _("20 min"), _("25 min"), _("30 min"),  _("35 min") };
wxString guard_zone_names[3] = { _("Off"), _("Arc"), _("Circle") };
static wxString sea_auto_names[4] = { _("Off"), _("Harbor"), _("Offshore"), _("Coastal") };
static wxString mbs_enabled_names[2] = { _("Off"), _("On") };

static SButtonSeq mainButtons[] = {
	{ ID_GAIN, _("Gain"), CT_GAIN, true, CControlButton::m_autoOff, 0 },
	{ ID_SEA, _("Sea clutter"), CT_SEA, true, CControlButton::m_autoOff, sea_auto_names },
	{ ID_RAIN, _("Rain clutter"), CT_RAIN, true, CControlButton::m_offOn, 0 },
	{ ID_FTC, _("FTC"), CT_FTC, true, CControlButton::m_offOn, 0 },
	{ 0, _(""), CT_MAX, false, 0, 0 }
};

static SButtonSeq advButtons[] = {
	{ ID_INTERFERENCE_REJECTION, _("Interference rejection"), CT_INTERFERENCE_REJECTION, false, 0, interference_rejection_names },
	{ ID_TARGET_BOOST, _("Target boost"), CT_TARGET_BOOST, false, 0, target_boost_names },
	{ ID_STC_PRESET, _("STC Preset"), CT_STC, false, 0, 0 },
	{ ID_TUNE, _("Fine Tune"), CT_TUNE_FINE, true, CControlButton::m_autoOff, 0 },
	{ ID_TUNE_COARSE, _("Coarse Tune"), CT_TUNE_COARSE, true, CControlButton::m_autoOff, 0 },
	{ ID_STC_CURVE, _("STC Curve"), CT_STC_CURVE, false, 0, 0 },
	{ 0, _(""), CT_MAX, false, 0, 0 },
};

static SButtonSeq installButtons[] = {
	{ ID_BEARING_ALIGNMENT, _("Bearing alignment"), CT_BEARING_ALIGNMENT, false, 0, 0 },
	{ ID_DISPLAY_TIMING, _("Display Timing"), CT_DISPLAY_TIMING, false, 0, 0 },
	{ ID_MBS_ENABLED, _("MBS"), CT_MBS_ENABLED, false, 0, mbs_enabled_names },
	{ 0, _(""), CT_MAX, false, 0, 0 },
};

void br24ControlsDialog::AddButtons(const SButtonSeq *buttons, wxBoxSizer *sizer, CButtonMap &buttonMap, int border)
{
	for(int i = 0; buttons[i].ctrl != CT_MAX; i++)
	{
		CControlButton *btn = 0;
		if(buttons[i].ctrl != CT_SEA)
		{
			btn = new CControlButton(this, buttons[i].id, buttons[i].label, buttons[i].ctrl, g_buttonSize,
							buttons[i].hasAuto, buttons[i].autoLabel, buttons[i].labels);
		}
		else
		{
			btn = new CControlButtonSea(this, buttons[i].id, buttons[i].label, buttons[i].ctrl, g_buttonSize,
							CT_SEA_AUTO, buttons[i].labels);
		}
		sizer->Add(btn, 0, wxALL, border);
		buttonMap.insert(CButtonMap::value_type(buttons[i].ctrl, *btn));
	}
}
...
#define BUTTON_HEIGTH_FUDGE 1 + 2 * BUTTON_BORDER
  ...
  AddButtons(advButtons, m_advanced_sizer, m_buttonMap, BORDER);
  ...
  AddButtons(installButtons, m_installation_sizer, m_buttonMap, BORDER);
  ...
  AddButtons(mainButtons, m_adjust_sizer, m_buttonMap, BORDER);
...
void br24ControlsDialog::OnPlusTenClick(wxCommandEvent& event) {
	if(m_changingControl != 0 && m_ri->m_radarControl != 0)
	{
		m_ri->m_radarControl->ChangeControlValue(m_changingControl->GetControlType(), 10);
	}
	else
	{
...
void br24ControlsDialog::OnPlusClick(wxCommandEvent& event) {
	if(m_changingControl != 0 && m_ri->m_radarControl != 0)
	{
		// fprintf(stderr, "OnPlusClick changing value of %d.\n", m_editControl);
		m_ri->m_radarControl->ChangeControlValue(m_changingControl->GetControlType(), 1);
	}
	else
	{
...
void br24ControlsDialog::OnAutoClick(wxCommandEvent& event) {
	if(m_changingControl != 0 && m_ri->m_radarControl != 0)
	{
		m_ri->m_radarControl->ToggleAuto(m_changingControl->GetControlType());
	}
	else
	{
...
void br24ControlsDialog::OnControlButtonClick(wxCommandEvent& event)
{
	CControlButton *cButton = (CControlButton *)event.GetEventObject();

	m_from_control = 0;
	m_changingControl = cButton;
	ControlType editControl = cButton->GetControlType();

	m_value_text->SetLabel(cButton->GetLabel());

	if (cButton->HasAuto()) 
	{
		m_auto_button->Enable(true);
		m_auto_button->Show();
		m_auto_button->SetLabel(cButton->GetAutoLabel());
	} 
	else 
	{
		m_auto_button->Hide();
		m_auto_button->Enable(false);
	}

	if(m_ri->m_radarControl != 0)
	{
		const CControlItem & item = m_ri->m_radarControl->GetControlValue(editControl);
		if(item.Max().IsSet() && item.Min().IsSet() && item.Max().Get() - item.Min().Get() > 20)
		{
			m_plus_ten_button->Show();
			m_minus_ten_button->Show();
		} 
...
void br24ControlsDialog::OnRadarStateButtonClick(wxCommandEvent& event) {
  ...
  switch (m_ri->m_state.value) {
    case RADAR_OFF:
      m_ri->m_radarControl->WakeupRadar();
...
void br24ControlsDialog::UpdateControlValues(bool refreshAll) {
  ...
  if (state == RADAR_TRANSMIT || state == RADAR_STANDBY) {
  ...
  if (m_ri->m_side_lobe_suppression.mod || refreshAll) {
	...
	if(m_ri != 0 && m_ri->m_radarControl != 0)
	{
		for(CButtonMap::iterator bmI = m_buttonMap.begin(); bmI != m_buttonMap.end(); bmI++)
		{
			bmI->second.SetLabel(bmI->second.GetLabel());
		}
	}
  ...
  if (m_from_control) {
	...
	if(m_changingControl != 0)
	{
		m_value_text->SetLabel(m_changingControl->GetLabel());
		if (m_changingControl->HasAuto()) 
		{
			// m_auto_button->Enable(true);
			// m_auto_button->Show();
			m_auto_button->SetLabel(m_changingControl->GetAutoLabel());
		} 
	}
}
...
void br24ControlsDialog::OnRefreshValues(wxCommandEvent &event)
{
	UpdateControlValues(false);
}
