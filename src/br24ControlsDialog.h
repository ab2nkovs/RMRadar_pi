...
const int ID_CONTROL_DIALOG_REFRESH = 100000;
...
class br24ControlsDialog : public wxDialog {
  ...
  DECLARE_EVENT_TABLE()
	...
	CButtonMap m_buttonMap;
	void AddButtons(const SButtonSeq *buttons, wxBoxSizer *sizer, CButtonMap &buttonMap, int border);
...
//  void OnRadarGainButtonClick(wxCommandEvent &event);
  ...
  void OnControlButtonClick(wxCommandEvent &event);
  ...
  void OnRefreshValues(wxCommandEvent &event);
  ...
  CControlButton *m_changingControl;
