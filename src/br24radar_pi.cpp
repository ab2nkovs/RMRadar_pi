...
wxString br24radar_pi::GetCommonName() { return wxT("RMRadar"); }

wxString br24radar_pi::GetShortDescription() { return _("Raymarin Radar PlugIn for OpenCPN"); }

wxString br24radar_pi::GetLongDescription() { return _("Raymarin Digital Radar PlugIn for OpenCPN\n"); }
...
void br24radar_pi::Notify(void) {
  ...
  if (m_pMessageBox->IsShown() || (m_settings.verbose != 0)) {
	...
	const SMiscRadarInfo & miscInfo = m_radar[r]->m_radarControl->GetMiscInfo();
        t << wxString::Format(wxT("%s\npackets %d/%d\nspokes %d/%d/%d\nmag curr %d SS %d\nrot time %d ms\n"), m_radar[r]->m_name.c_str(),
                              ...
                              m_radar[r]->m_statistics.missing_spokes, miscInfo.m_magnetronCurrent, 
			      miscInfo.m_signalStrength, miscInfo.m_rotationPeriod);
      }
    }
...
bool br24radar_pi::LoadConfig(void) {
  ...
  if (pConf) {
    ...
    pConf->SetPath(wxT("/Plugins/RMRadar"));
    ...
    pConf->Read(wxT("RadarControlActive"), &m_settings.enable_transmit, false);
...
bool br24radar_pi::SaveConfig(void) {
  ...
  if (pConf) {
    pConf->DeleteGroup(wxT("/Plugins/RMRadar"));
    pConf->SetPath(wxT("/Plugins/RMRadar"));
    ...
    pConf->Write(wxT("RadarControlActive"), m_settings.enable_transmit);
