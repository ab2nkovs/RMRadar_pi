...
static const RadarRange g_ranges_metric[] = {
    ...
    {201, 403, "1/8 SM", 0, "1/16 SM", 0},
    {250, 500, "250 m", 0, "125", 0},
    {401, 803, "1/4 SM", 0, "1/8 SM", 0},
    {500, 1000, "500 m", "125", "250", "375"},
    {750, 1500, "750 m", 0, "375", 0},
    {803, 1607, "1/2 SM", "1/8 SM", "1/4 SM", "3/8 SM" },
    {1207, 2415, "3/4 SM", 0, "3/8 SM", 0 },
    {1500, 3000, "1.5 km", 0, "750", 0},
    {2413, 4826, "1.5 SM", "3/8 SM", "3/4 SM", 0 },
    {3000, 6000, "3 km", 0, "1500", 0},
    {4828, 9656, "3 SM", 0, "1.5 SM", 0 },
    {6000, 12000, "6 km", "1.5", "3.0", "4.5"},
    {9656, 19312, "6 SM", "1.5 SM", "3 SM", "4.5 SM" },
    {11999, 23998, "12 km", "3", "6", "9"},
    {19310, 38621, "12 SM", "3 SM", "6 SM", "9 SM" },
    {24000, 48000, "24 km", "6", "12", "18"},
    {48000, 96000, "48 km", "12", "24", "36"},
    {77246, 154493, "48 SM", "12 SM", "24 SM", "36 SM" },
    {96000, 192000, "96 km", "24", "48", "72" },
    {115870, 231740, "72 SM", "18 SM", "36 SM", "54 SM" },
    {144000, 288000, "144 km", "36", "72", "108" }
};

static const RadarRange g_ranges_nautic[] = {
	{1852 / 8, 1852 / 4, "1/8 NM", 0, "1/16", 0},
	{1852 / 4, 1852 / 2, "1/4 NM", "1/16", "1/8", 0},
	{1852 / 2, 1852, "1/2 NM", "1/8", "1/4", "3/8"},
	{1852 * 3 / 4, 1852 * 3 / 2, "3/4 NM", 0, "3/8", 0},
	{1852 * 3 / 2, 1852 * 3, "1.5 NM", "3/8", "3/4", 0},
	{1852 * 3, 1852 * 6, "3 NM", 0, "1.5", 0},
	{1852 * 6, 1852 * 12, "6 NM", "1.5", "3", "4.5"},
	{1852 * 12, 1852 * 24, "12 NM", "3", "6", "9"},
	{1852 * 24, 1852 * 48, "24 NM", "6", "12", "18"},
	{1852 * 48, 1852 * 96, "48 NM", "12", "24", "36"},
	{1852 * 72, 1852 * 144, "72 NM", "18", "36", "54"}
};
...
void RadarInfo::StartReceive() {
  if (!m_radarControl) {
    ...
    m_radarControl = new CRMControl(m_pi, this);
    if (!m_radarControl || (m_radarControl->Run() != wxTHREAD_NO_ERROR)) {
...
void RadarInfo::AdjustRange(int adjustment) {
  ...
  if (m_range.range) {
    ...
    if (adjustment > 0 && m_range.range > min) {
      ...
      m_radarControl->SetRangeMeters(m_range.range[-1].actual_meters);
  ...
  if (m_state.value == RADAR_TRANSMIT && m_auto_range_mode) {
    ...
    if (test < 95 || test > 105) {  //   range change required
      ...
      if (meters != m_range_meters) {
        ...
        m_radarControl->SetRangeMeters(meters);
...
bool RadarInfo::SetControlValue(ControlType controlType, int value) { return m_radarControl->SetControlValue(controlType, value); }
...
wxString RadarInfo::GetCanvasTextCenter() {
  ...
  switch (m_state.value) {
    ...
    case RADAR_STANDBY:
      ...
      break;
    case RADAR_WAKING_UP:
      s << _("Radar is waking up\n(") << m_radarControl->GetMiscInfo().m_warmupTime << _(" sec)");
      break;
    case RADAR_TRANSMIT:
      if (m_draw_panel.draw) {
        return s;
...
wxString &RadarInfo::GetRangeText() {
  ...
  if (!r) {
    m_range_text = wxT("Units not set");
    return m_range_text;
  }
